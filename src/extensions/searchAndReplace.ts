...
function processSearches (doc: PMNode, searchTerm: RegExp, searchResultClass: string,currentIndex:number=0): ProcessedSearches {
  ...
  for (let i = 0; i < results.length; i += 1) {
    const r = results[i]
    if(i===currentIndex){
      decorations.push(Decoration.inline(r.from, r.to, { class: searchResultClass+' current-result' }))
    }else{
        ...
        state: {
          ...
          apply ({ doc, docChanged }, oldState) {
            ...
            const {
              decorationsToReturn,
              results,
            } = processSearches(doc, getRegex(searchTerm, disableRegex, caseSensitive), searchResultClass,editor.storage.index)
